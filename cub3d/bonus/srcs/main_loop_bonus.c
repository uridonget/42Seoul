...
/*   Updated: 2024/05/14 16:03:39 by haejeong         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	check_and_move(double step_x, double step_y, t_museum *mu)
{
	...
	if (mu->map.map[(int)mu->ray.pos_y / 64][(int)temp_x / 64] == '1'
		|| mu->map.map[(int)mu->ray.pos_y / 64][(int)temp_x / 64] == 'D')
		return ;
	if (step_y > 0)
	...
	if (mu->map.map[(int)temp_y / 64][(int)mu->ray.pos_x / 64] == '1'
		|| mu->map.map[(int)temp_y / 64][(int)mu->ray.pos_x / 64] == 'D')
		return ;
	if (mu->map.map[(int)temp_y / 64][(int)temp_x / 64] == '1'
		|| mu->map.map[(int)temp_y / 64][(int)temp_x / 64] == 'D')
		return ;
	mu->ray.pos_x += step_x;
	mu->ray.pos_y += step_y;
}
